...
`~/Library/Preferences/com.apple.messages.pinning.plist` read-only, and drive
Messages.app through AppleScript. The pinning plist contributes only the
...
details from the contact resolver, attachment bytes you request, and files you send. Push notifications use a
