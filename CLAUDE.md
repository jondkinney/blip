...
- **Pins belong to Messages.app.** The ordered pin list lives in
  ...
  bounded pD/pP tokens to canonical chat identifiers and emits only
  `pinned_order`; Blip must not create a divergent Linux-only favorites list.
...
- **Ambiguous contact names are explicit choices, never guesses.**
  ...
  Contacts.app saves. Gated narrow repair can remove an exact revalidated
  ...
  AddressBook SQLite database. Edits preserve the target photo; deletion and
  consolidation receipts cannot guarantee restored account placement, links,
  or photos, so the destructive confirmation must keep saying so.
